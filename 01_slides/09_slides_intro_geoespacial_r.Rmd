...
map_dem_rc_ggplot2 <- ggplot() +
  ...
  geom_sf(data = rc_2020, color = "red", fill = NA, size = 1.3) +
...
tm_shape(biomas) +
  ...
  tm_credits("CRS: SIRGAS2000/Geo", position = c(.63, .13)) +
  tm_credits("Fonte: IBGE (2019)", position = c(.63, .09)) +
  tm_layout(main.title = "Biomas do Brasil", title.position = c(.25, .95),
            title.size = 1.8, title.fontface = "bold",
...
# exportar
tmap::tmap_animation(
  tm = map_brasil_tmap_ani, 
  filename = here::here("03_dados", 
  "mapas", 
  "mapa_dem_rc_tmap_ani.gif"), 
  delay = 30)
```
